...
public class TurretOperatorHatLayer<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack ms, MultiBufferSource buffer, int light, LivingEntity entity, float limbSwing, float limbSwingAmount,
        ...
        if (entityModel instanceof AgeableListModel<?> model && model instanceof com.jesz.createdieselgenerators.mixins.AgeableListModelAccessor modelA) {
            if (model.young) {
                ...
                ms.translate(0.0D, modelA.getBabyYHeadOffset() / 16.0F, modelA.getBabyZHeadOffset() / 16.0F);
            }
        ...
        if (!partsToHead.isEmpty()) {
            ...
            if (!lastChild.isEmpty()) {
                ModelPart.Cube cube = lastChild.getRandomCube(RandomSource.create(1023112312));
            ...
            CachedBuffers.partial(CDGPartialModels.TURRET_OPERATOR_HAT, air)
                    .light(light)
    ...
    private boolean shouldRenderOn(LivingEntity entity) {
        if (entity == null)
            return false;
        if (((IEntity)entity).getTurretPos() != null)
            return true;
        return false;
    }
    ...
    public static void registerOn(EntityRenderer<?> entityRenderer) {
        ...
        livingRenderer.addLayer((TurretOperatorHatLayer) new TurretOperatorHatLayer<>(livingRenderer));
    }
